...
# Theme
#
# target: theme                   - Run yarn or npm build command.
.PHONY: theme
theme:
	@$(call HELPTEXT,$@)
	[ -d theme ] && (yarn build || npm run build)

# target: theme-watch             - Run yarn or npm watch command.
.PHONY: theme-watch
theme-watch:
	@$(call HELPTEXT,$@)
	[ -d theme ] && (yarn watch || npm run watch)


# target: theme-install           - Install modules with yarn or npm.
.PHONY: theme-install
theme-install:
	@$(call HELPTEXT,$@)
	[ -d theme ] && (yarn install || npm install)
